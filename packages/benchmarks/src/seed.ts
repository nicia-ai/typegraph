...
function buildFollowEdges(
  ...
  for (let fromIndex = 0; fromIndex < userCount; fromIndex += 1) {
    ...
    while (seen.size < followsPerUser) {
      const skew = rng();
      const candidate =
...
async function ingestUsers(
  ...
  for (
    ...
    await store.nodes.User.bulkCreate(
      batch.map((user) => ({
        id: user.id,
        props: {
          name: user.name,
...
async function ingestPosts(
  ...
  for (
    ...
    await store.nodes.Post.bulkCreate(
      batch.map((post) => ({
        id: post.id,
        props: {
          title: post.title,
...
async function ingestFollowEdges(
  ...
  for (
    ...
    await store.edges.follows.bulkCreate(
      batch.map((edge) => ({
...
async function ingestNextEdges(
  ...
  for (
    ...
    await store.edges.next.bulkCreate(
      batch.map((edge) => ({
        from: { kind: "User" as const, id: edge.fromId },
        to: { kind: "User" as const, id: edge.toId },
        props: {},
