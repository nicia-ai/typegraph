...
function installLocalSqliteBaseSchema(
  ...
  // v2 (the fences relation), v3 (the recorded-relations `since_idx`
  // indexes), and v4 (the recorded identity-assertions relation's own
  // `since_idx`) need no adoption logic beyond what
  // `generateSqliteMigrationSQL` already emits: a brand-new relation or
  // index is fully covered by its own `CREATE TABLE IF NOT EXISTS` /
  // `CREATE INDEX IF NOT EXISTS`, on both a fresh database and one that
  // already has every OTHER base table or index. Only v1's edge-match-
  // identity `ADD COLUMN` migration — handled by the catch block below —
  // needed runtime introspection this synchronous path writes by hand
  // instead of running `BaseSchemaLifecycle`'s async state machine.
