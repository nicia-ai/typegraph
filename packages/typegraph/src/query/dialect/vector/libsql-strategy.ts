...
const LIBSQL_CAPABILITIES: VectorCapabilities = {
  ...
  // pgvector >= 0.8 (iterative scan), libSQL has no recovery.
  filteredApproximateSearch: "post-filter",
};
...
export const libsqlVectorStrategy: VectorStrategy = {
  ...
  buildSearch(slot, params: VectorSearchParams, candidates?: SQL): SQL {
    ...
    if (usesAnnIndex(slot)) {
      ...
      // `capabilities.filteredApproximateSearch === "post-filter"`.
 ...
 * meaning of `capabilities.filteredApproximateSearch === "post-filter"`.
 */
