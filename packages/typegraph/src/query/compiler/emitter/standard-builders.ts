...
export type StandardEmitterPredicateIndex = Readonly<{
...
function compileSelectiveProjection(
  ...
  const columns = fields.map((field) => {
    ...
    if (field.isSystemField) {
      const dbColumn =
        field.field === "fromId" ? "from_id"
        : field.field === "toId" ? "to_id"
        : field.field.startsWith("meta.") ?
          field.field
            .slice(5)
...
export function buildStandardVectorOrderBy(
  ...
  const { ast, dialect, vectorPredicate } = input;
  void vectorPredicate;
