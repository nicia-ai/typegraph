...
/**
 * The temporal coordinate a {@link StoreView} pins. `asOf` is required
 * when `mode` is `"asOf"` and ignored otherwise.
 */
export type StoreViewCoordinate = Readonly<{
  /** The temporal mode the view reads in. */
  mode: TemporalMode;
  /** ISO-8601 timestamp — required when `mode` is `"asOf"`. */
  asOf?: string;
}>;
