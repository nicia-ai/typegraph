...
function deriveOptionalGraphBackendMembers(
  ...
  if (optionalNames.length !== 85) {
    throw new Error(
      `Expected exactly 85 optional GraphBackend members (the scan's own precondition); found ${optionalNames.length}. GraphBackend's optional surface has changed — re-derive every partition constant before trusting this scan.`,
    );
  }
