 ...
 *
 * A candidate node's member name matching one of the 96 optional members is
...
export type BundleMemberAccessScan = Readonly<{
  ...
  /** Live count per member name, over every one of the 96 optional members (0 included). */
...
/**
 * The 96 optional `GraphBackend` members, read off the CHECKER rather than
 * hand-copied — a member added or removed from `GraphBackend` changes this
 * set automatically. The `size === 96` assertion is the scan's own
...
function deriveOptionalGraphBackendMembers(
  ...
  if (optionalNames.length !== 96) {
    throw new Error(
      `Expected exactly 96 optional GraphBackend members (the scan's own precondition); found ${optionalNames.length}. GraphBackend's optional surface has changed — re-derive every partition constant before trusting this scan.`,
    );
  }
