...
export type BundleMemberAccessScan = Readonly<{
  ...
  /** Live count per member name, over every one of the 91 optional members (0 included). */
...
/**
 * The 94 optional `GraphBackend` members, read off the CHECKER rather than
 * hand-copied — a member added or removed from `GraphBackend` changes this
 * set automatically. The `size === 94` assertion is the scan's own
...
function deriveOptionalGraphBackendMembers(
  ...
  if (optionalNames.length !== 95) {
    throw new Error(
      `Expected exactly 95 optional GraphBackend members (the scan's own precondition); found ${optionalNames.length}. GraphBackend's optional surface has changed — re-derive every partition constant before trusting this scan.`,
    );
  }
