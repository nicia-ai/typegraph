/**
 * Acceptance gate for the public new-vs-live-target entry point `mergeIncremental()`
 * (design §6.4-B / §6.6). It commits branch additions, resolves them against the
 * committed `target` (base-id-wins), propagates inherited modifications/deletions,
 * and refuses row hazards that would resurrect tombstones or corrupt edge identity.
 *
 ...
 *   - inherited modify / delete propagate when the live target did not conflict;
 *   - live target modify/delete wins over stale branch delete/modify conflicts;
...
describe.each(backendMatrix())(
  ...
  (entry) => {
    ...
    async function seedCarePath(store: CareStore) {
      ...
      const [edge] = await store.edges.hadEncounter.bulkCreate([
        {
          id: "edge-1",
    ...
    it("APPLIES an inherited node modification when the target is unchanged", async () => {
      cleanups = [];
      const forkPoint = await emptyStore();
    ...
    it("APPLIES an inherited node deletion when the target is unchanged", async () => {
      ...
      expect(isOk(result)).toBe(true);
      expect(await target.nodes.Patient.find()).toEqual([]);
    });

    it("APPLIES an inherited edge modification when the target is unchanged", async () => {
      cleanups = [];
      const forkPoint = await emptyStore();
      ...
      expect(edge?.on).toBe("2026-06-15");
    });

    it("APPLIES an inherited edge deletion when the target is unchanged", async () => {
      cleanups = [];
      const forkPoint = await emptyStore();
      const baseEdge = await seedCarePath(forkPoint);
      const provider = await forkOf(forkPoint);
    ...
    it("TARGET WINS: target modification survives a branch deletion", async () => {
      ...
      const result = await mergeIncremental<CareGraph>({
        ...
        target,
        branches: [provider],
        options: options({ onDeleteModifyConflict: "deleteWins" }),
      });
      expect(isOk(result)).toBe(true);
      if (!isOk(result)) {
        return;
      ...
      const [patient] = await target.nodes.Patient.find();
      expect(patient?.tag).toBe("target-live");
      expect(result.data.deleteModifyConflicts[0]?.resolution).toBe(
        "modifyWins",
      );
    });

    it("TARGET WINS: target deletion is not resurrected by a branch modification", async () => {
      cleanups = [];
      const forkPoint = await emptyStore();
      const [base] = await forkPoint.nodes.Patient.bulkCreate([
        { id: "base-ana", props: { name: "Anna Rivera", mrn: "MRN-1" } },
    ...
    it("TARGET WINS: preserves a colliding committed NODE id", async () => {
      cleanups = [];
      const forkPoint = await emptyStore();
      const provider = await forkOf(forkPoint);
      // A novel patient (mrn has no base match) under an id that the live target
      // already owns. Same `(kind,id)` is one identity; target props win.
      await provider.store.nodes.Patient.bulkCreate([
      ...
      const [patient] = await target.nodes.Patient.find();
      expect(patient?.name).toBe("Committed Person");
      expect(patient?.mrn).toBe("MRN-OTHER");
      const conflictProperties = result.data.conflicts
        .map((conflict) => conflict.property)
        .sort();
      expect(conflictProperties).toEqual(["mrn", "name"]);
    });
    ...
    it("GUARD (kind-aware): a base member does NOT exempt a same-id node of another kind", async () => {
      ...
      // Patient:shared is the base member; Encounter:shared is a separate same-id
      // target identity and must be preserved rather than overwritten by the branch.
      expect(isOk(result)).toBe(true);
      if (!isOk(result)) {
        return;
      }
      const encounter = (await target.nodes.Encounter.find()).find(
        (item) => item.id === "shared",
      );
      expect(encounter?.reason).toBe("committed");
    });
