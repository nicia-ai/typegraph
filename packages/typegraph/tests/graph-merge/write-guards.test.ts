...
describe("provenanceNodeId (cross-kind identity)", () => {
  ...
  it("distinguishes two different-kind contributions that share a bare id", async () => {
    const patient = await provenanceNodeId("g", base("Patient"));
    const encounter = await provenanceNodeId("g", base("Encounter"));
    expect(patient).not.toBe(encounter);
  });

  it("is stable for the same record (deterministic upsert key)", async () => {
    expect(await provenanceNodeId("g", base("Patient"))).toBe(
      await provenanceNodeId("g", base("Patient")),
    );
  });
});
