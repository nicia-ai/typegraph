...
describe("normalizeMergeOptions validation", () => {
  ...
  it('rejects "provenanceWeighted" with no provenanceWeights', () => {
    expect(() =>
      normalizeMergeOptions<G>({ onPropertyConflict: "provenanceWeighted" }),
    ).toThrow(/provenanceWeights/);
  });

  it('rejects "provenanceWeighted" with an empty provenanceWeights map', () => {
    expect(() =>
      normalizeMergeOptions<G>({
  ...
  it('accepts "provenanceWeighted" when weights are provided', () => {
    expect(() =>
      normalizeMergeOptions<G>({
        onPropertyConflict: "provenanceWeighted",
        provenanceWeights: new Map([[asBranchId("b1"), 1]]),
      }),
    ).not.toThrow();
  });
