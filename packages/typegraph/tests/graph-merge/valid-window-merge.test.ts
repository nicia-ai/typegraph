...
describe.each(backendMatrix())("merging valid-time windows [$name]", (entry) => {
  ...
  async function nodeEnd(
    ...
    return canonicalizeDatabaseTimestamp((await nodeRow(store, kind, id)).valid_to);
  }
  ...
  function unapplicableIds(report: MergeReport<CareGraph>): readonly string[] {
  ...
  it("carries a window-only ending from a branch to the target", async () => {
    ...
    expect(report.validityEnds).toEqual([
      { entity: "node", kind: "Patient", id: "pat-1", validTo: LATE, claimedBy: [BRANCH_A] },
      { entity: "edge", kind: "hadEncounter", id: "edge-1", validTo: LATE, claimedBy: [BRANCH_A] },
    ]);
  });
  ...
  it("keeps the incremental target's own end over a branch's", async () => {
    const forkPoint = await seededForkPoint();
    const target = (await forkOf(forkPoint, asBranchId("window-target"))).store;
    const branchA = await forkOf(forkPoint, BRANCH_A);
  ...
  it("does not write when a branch re-states the end the target holds", async () => {
    ...
    const target = await seed(await newStore({ coalesceUnchangedUpserts: true }));
    await target.nodes.Patient.update(PATIENT, {}, { validTo: LATE });
  ...
  it("lets a single branch extend an end to a later instant", async () => {
    ...
    const branchA = await forkOf(forkPoint, BRANCH_A);
    await branchA.store.nodes.Patient.update(PATIENT, {}, { validTo: LATEST });
  ...
  it("leaves the target's window alone for a props-only modification", async () => {
    const forkPoint = await seededForkPoint({ coalesceUnchangedUpserts: true });
    await forkPoint.nodes.Patient.update(PATIENT, {}, { validTo: LATE });
    const branchA = await forkOf(forkPoint, BRANCH_A);
  ...
  it("does not rewrite an unchanged row when nothing changed at all", async () => {
    const forkPoint = await seededForkPoint({ coalesceUnchangedUpserts: true });
    await forkPoint.nodes.Patient.update(PATIENT, {}, { validTo: LATE });
    const versionBefore = (await nodeRow(forkPoint, "Patient", "pat-1")).version;
    const branchA = await forkOf(forkPoint, BRANCH_A);
    await branchA.store.nodes.Encounter.update(ENCOUNTER, { reason: "follow-up" });
