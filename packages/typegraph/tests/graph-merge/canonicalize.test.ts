...
describe("ClusterMember origin discriminator (step 1)", () => {
  ...
  it("keeps the committed base VALUE under the default 'flag' base policy (#4)", () => {
    // base-id-wins governs IDENTITY; the base VALUE is governed by
    // onBasePropertyConflict. The default 'flag' keeps the committed value and the
    // base sentinel is NOT a real branch, so it never appears in `values`.
    const cluster = clusterOf("a-new", "z-base");
    const members: readonly ClusterMember[] = [
      {
        origin: "base",
  ...
  it("lets an explicit non-flag base policy overwrite the base VALUE while keeping its identity (#4 opt-in)", () => {
    // onBasePropertyConflict:'lastWriteWins' is the documented opt-in to let a
    // branch overwrite committed data. The committed IDENTITY (base-id-wins) is
    // still preserved; only the conflicting property value is overwritten.
    const cluster = clusterOf("a-new", "z-base");
    const members: readonly ClusterMember[] = [
      {
        origin: "base",
    ...
    const entity = canonicalizeCluster(
      ...
      rank([b1]),
      undefined, // weights
      undefined, // canonicalOverride
      "lastWriteWins", // basePolicy — explicit opt-in to overwrite the base value
    );
