...
describe.runIf(process.env["POSTGRES_URL"])(
  ...
  () => {
    ...
    it("indexes searchable fields and finds them via tsvector", async () => {
      ...
      expect(asDocument(requireDefined(results[0])).title).toBe(
    ...
    it("supports websearch syntax with negation and phrase", async () => {
      ...
      expect(asDocument(requireDefined(negated[0])).title).toBe("Apple recipe");
      ...
      expect(asDocument(requireDefined(phrase[0])).title).toBe("Apple recipe");
    });
