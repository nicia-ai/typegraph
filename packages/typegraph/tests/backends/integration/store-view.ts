  ...
  describe("StoreView (read-only as-of lens)", () => {
    ...
    describe("construction", () => {
      it("requires a timestamp for asOf mode", () => {
        const store = context.getStore();
        expect(() => store.view({ mode: "asOf" })).toThrow(ValidationError);
      });
      ...
      it("rejects an asOf paired with a non-asOf mode instead of dropping it", () => {
        ...
        // and the pin ignored). It is now a ValidationError, not a silent no-op.
        expect(() => store.view({ mode: "current", asOf: AS_OF })).toThrow(
          ValidationError,
        );
        expect(() => store.view({ mode: "includeEnded", asOf: AS_OF })).toThrow(
          ValidationError,
        );
        expect(() =>
