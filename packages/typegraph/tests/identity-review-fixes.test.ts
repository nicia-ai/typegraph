...
describe("identity review fixes", () => {
  ...
  it("#18 no-op identity mutations do not advance the revision clock", async () => {
    ...
    // The transaction-scoped revision coalescer must preserve those no-op
    // signals instead of treating every nested mutation as a write.
    await store.transaction(async (tx) => {
      await tx.identity.retractAssertion(
        asIdentityAssertionId("does-not-exist"),
      );
  ...
  it("chunks identity assertion and recorded-history writes by the backend bind budget", async () => {
    ...
    try {
      ...
      expect(requireDefined(rows[0]).total).toBe(8);
    } finally {
