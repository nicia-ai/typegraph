...
The backend planning API uses one `executeManagedCreate` contract for node and edge creates. Custom backends retain the portable write fallback, while existing persisted claim rows remain supported.
